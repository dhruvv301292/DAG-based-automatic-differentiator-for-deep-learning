...
class GRUUnit(Module):
    ...
    def forward(self, input, hidden = None):
        ...
        z_t = sigmoid((hidden @ self.weight_hz.T()) + self.bias_hz + (input @ self.weight_iz.T()) + self.bias_iz)
        r_t = sigmoid((hidden @ self.weight_hr.T()) + self.bias_hr + (input @ self.weight_ir.T()) + self.bias_ir)
        # h_t = tanh(((r_t * hidden) @ self.weight_hn.T()) + self.bias_hn + (input @ self.weight_in.T()) + self.bias_in)
        n_t = tanh((input @ self.weight_in.T()) + self.bias_in + (r_t * ((hidden @ self.weight_hn.T()) + self.bias_hn)))
        hidden = ((tensor.Tensor(np.array([1.])) - z_t) * n_t) + (z_t * hidden)
        return hidden
